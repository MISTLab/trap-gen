...
# they are general purpose registers, in particular:
# GPR[0]: Always has a value of zero. Anything written to R0 is discarded.
# GPR[1-13]: general purpose registers
# GPR[14]: used to store return addresses from interrupts.
# GPR[15]: Recommended for storing return addresses from user vectors.
# GPR[16]: Used to store return adrresses from breaks.
# GPR[17]: If microblaze is configured to support hardware exceptions, thyis register is loaded with the address of the instructions following the instruction causing the HW exception, except for ...
# GPR[18-31]: general purpose register

regBank = trap.RegisterBank('GPR', 32, 32)
processor.addRegBank(regBank)
regBank.setConst(0,0)

# A registry bank of 18 special purpouse registers each one 32 bits wide
#Do we have to use a bitmask for each register?
