...
print '\nCompiling binutils\n'
if os.system('cd ' + os.path.abspath(binutils + '_build') + ' && CC=i686-mingw32-gcc CXX=i686-mingw32-g++ ' + os.path.abspath(binutilsName + '/configure') + ' --host=i686-mingw32 --build=i686-pc-linux-gnu --target=' + targetArch + ' --prefix=' + os.path.abspath(prefix) + ' --enable-multilib ' + addFlags + ' && make -j' + str(numProc) + ' && make install') != 0:
    sys.exit()
...
if os.system('export PATH=' + os.path.abspath(nativeToolchain + '/bin') + ':$PATH && cd ' + os.path.abspath(gcc + '_build') + ' && make -j' + str(numProc) + ' && make install') != 0:
    sys.exit()
