...
while not os.path.exists(nativeToolchain):
    print 'path --> ' + nativeToolchain + ' <-- not existing, please specify an existing one'
    nativeToolchain = (raw_input('Please specify the native toolchain path: ')).replace('\n', '')
...
print '\nCompiling binutils\n'
if os.system('cd ' + os.path.abspath(binutils + '_build') + ' && CC=i686-mingw32-gcc CXX=i686-mingw32-g++ ' + os.path.abspath(binutilsName + '/configure') + ' --host=i686-mingw32 --build=i686-pc-linux-gnu --target=' + targetArch + ' --prefix=' + os.path.abspath(prefix) + ' --enable-multilib ' + addFlags + ' && make -j2 && make install') != 0:
    sys.exit()
print '\nCompiling gcc step-1\n'
if os.system('export PATH=' + os.path.abspath(nativeToolchain + '/bin') + ':$PATH && cd ' + os.path.abspath(gcc + '_build') + ' && CC=i686-mingw32-gcc CXX=i686-mingw32-g++ ' + os.path.abspath(gccName + '/configure') + ' --host=i686-mingw32 --build=i686-pc-linux-gnu --target=' + targetArch + ' --prefix=' + os.path.abspath(prefix) + ' --enable-multilib --with-newlib --with-tls --with-__thread --enable-languages=\'c,c++\' --with-headers=' + os.path.abspath(newlibName + '/newlib/libc/include') + ' --disable-__cxa_atexit --disable-__dso_handle ' + addFlags + ' && make all-gcc -j2 && make install-gcc') != 0:
    sys.exit()
...
print '\nCompiling newlib\n'
if os.system('export PATH=' + os.path.abspath(nativeToolchain + '/bin') + ':$PATH && cd ' + os.path.abspath(newlib + '_build') + ' && CC=i686-mingw32-gcc CXX=i686-mingw32-g++ ' + os.path.abspath(newlibName + '/configure') + ' --host=i686-mingw32 --build=i686-pc-linux-gnu --target=' + targetArch + ' --prefix=' + os.path.abspath(prefix) + ' --enable-multilib ' + addFlags + ' && make -j2 && make install') != 0:
    sys.exit()
print '\nCompiling gcc step-2\n'
if os.system('export PATH=' + os.path.abspath(nativeToolchain + '/bin') + ':$PATH && cd ' + os.path.abspath(gcc + '_build') + ' && make -j2 && make install') != 0:
    sys.exit()
#Now it is time to see if we need to cross-compiler GDB
print '\nCompiling debugger\n'
if os.path.exists(insight):
    if os.system('export PATH=' + os.path.abspath(nativeToolchain + '/bin') + ':$PATH && cd ' + os.path.abspath(insight + '_build') + ' && CC=i686-mingw32-gcc CXX=i686-mingw32-g++ ' + os.path.abspath(insightName + '/configure') + ' --host=i686-mingw32 --build=i686-pc-linux-gnu --target=' + targetArch + ' --prefix=' + os.path.abspath(prefix) + ' --enable-multilib ' + addFlags + ' && make -j2 && make install') != 0:
        sys.exit()
elif os.path.exists(gdb):
    if os.system('export PATH=' + os.path.abspath(nativeToolchain + '/bin') + ':$PATH && cd ' + os.path.abspath(gdb + '_build') + ' && CC=i686-mingw32-gcc CXX=i686-mingw32-g++ ' + os.path.abspath(gdbName + '/configure') + ' --host=i686-mingw32 --build=i686-pc-linux-gnu --target=' + targetArch + ' --prefix=' + os.path.abspath(prefix) + ' --enable-multilib ' + addFlags + ' && make -j2 && make install') != 0:
        sys.exit()
