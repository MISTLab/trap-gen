...
class TestFileDumper(unittest.TestCase):
    ...
    def testDumpAll(self):
        ...
        self.assertEqual(lines[5], 'def build(bld):\n')
        self.assertEqual(lines[6], '    obj = bld.new_task_gen(\'cxx\', \'program\')\n')
        self.assertEqual(lines[7], '    obj.source=\"\"\"\n')
        self.assertEqual(lines[8], '        prova.cpp\n')
        self.assertEqual(lines[9], '    \"\"\"\n')
        self.assertEqual(lines[10], '    obj.uselib = \'BOOST BOOST_THREAD BOOST_FILESYSTEM BOOST_SYSTEM SYSTEMC TLM TRAP\'\n')
        self.assertEqual(lines[11], '    obj.includes = \'.\'\n')
        self.assertEqual(lines[12], '    obj.name = \'temp\'\n')
        self.assertEqual(lines[13], '    obj.target = \'temp\'\n')
        import shutil
        shutil.rmtree('temp', True)
